...
if uploaded_file is not None:
    ...
    nice_name = clean_name(disease_name)

    st.subheader(f"🧪 Disease Prediction: `{nice_name}`")
    st.write(f"🔍 Confidence: `{confidence:.2f}`")

    # Save for chatbot
    st.session_state["detected_disease"] = nice_name

    if st.button("💬 Get AI Advice for this Disease"):
...
selected_sample = st.selectbox("Choose a sample image:", ["None"] + list(sample_images.keys()))

if selected_sample != "None":
    sample_image = Image.open(sample_images[selected_sample])
    st.image(sample_image, caption=selected_sample, use_container_width=True)

    disease_name, confidence = predict(sample_image)
    nice_name = clean_name(disease_name)

    st.subheader(f"🧪 Prediction: `{nice_name}`")
    st.write(f"🔍 Confidence: `{confidence:.2f}`")
