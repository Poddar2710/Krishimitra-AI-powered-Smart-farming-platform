...
if st.button("Get Weather"):
    ...
    if response.status_code != 200:
        fallback_city = fallback_map.get(selected_city)

        if fallback_city:
            params["q"] = f"{fallback_city},IN"
            response = requests.get(base_url, params=params)
    # --- Process Response ---
    if response.status_code == 200:
        data = response.json()
        ...
        st.success(f"📍 Weather in {selected_city}, Bihar")

        st.write(f"🌡️ Temperature: **{temperature}°C**")
        st.write(f"🌥️ Condition: **{weather_desc.title()}**")
        ...
        if show_advice:
            ...
            advice_list = get_weather_advice(
                temperature, humidity, weather_desc, wind_speed
            )
            ...
            if advice_list:
                ...
                st.info("✅ Conditions look normal for farming.")

    else:
        st.error("❌ Unable to fetch weather for this district.")
